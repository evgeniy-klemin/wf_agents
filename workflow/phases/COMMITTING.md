...
1. **Instruct Developer to commit and push** with a meaningful commit message:
...
2. **Wait for Developer confirmation** that commit and push are done
3. **Verify:** git status (working tree must be clean)
4. **Verify:** branch is up to date with remote branch
5. **Decide:** more iterations or all done?
  - More iterations → {{WF_CLIENT}} transition <id> --to RESPAWN --reason "Iteration N+1: <task>" --repo {WORKTREE}
  - All done → {{WF_CLIENT}} transition <id> --to PR_CREATION --reason "All iterations complete" --repo {WORKTREE}
